...
PyObject *LOOKUP_ATTRIBUTE(PyThreadState *tstate, PyObject *source, PyObject *attr_name) {
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        if (dict == NULL) {
// Nitro: when LayoutOracle has probed the dict-pointer layout, use the
// compile-time offset directly instead of loading tp_dictoffset at runtime.
// These defines are set by nuitka_offsets.h (generated by LayoutOracle).
// This code is inside hasTypeGenericGetAttr(), which already verified the
// type uses PyObject_GenericGetAttr, and we are past the data-descriptor
// check above — so the dict-first lookup order is correct here.
#if defined(__NUITKA_DMA_ACTIVE__) && defined(NUITKA_HAS_FIXED_DICT_OFFSET)
            // Compile-time offset: no tp_dictoffset load.
            PyObject **dict_pointer = (PyObject **)((char *)source + (NITRO_FIXED_OFFSET));
            dict = *dict_pointer;
            ...
            {
                PyObject **dict_pointer = _PyObject_GetDictPtr(source);
...
PyObject *LOOKUP_ATTRIBUTE_WEAKREF_SLOT(PyThreadState *tstate, PyObject *source) {
    CHECK_OBJECT(source);

    PyTypeObject *type = Py_TYPE(source);

    if (_PyType_SUPPORTS_WEAKREFS(type)) {
#if defined(__NUITKA_DMA_ACTIVE__)
        PyObject **weakref_list_ptr = Nuitka_Nitro_GetWeakrefListPtr(source);
#else
        PyObject **weakref_list_ptr = _PyObject_GET_WEAKREFS_LISTPTR(source);
#endif

        if (weakref_list_ptr != NULL && *weakref_list_ptr != NULL) {
            PyObject *result = *weakref_list_ptr;
            Py_INCREF(result);
            return result;
        }
    }
...
int BUILTIN_HASATTR_BOOL(PyThreadState *tstate, PyObject *source, PyObject *attr_name) {
    ...
    // Use optimized HAS_ATTR_BOOL2 logic.
    return HAS_ATTR_BOOL2(tstate, source, attr_name);
}
...
static bool SET_ATTRIBUTE_GENERIC(PyThreadState *tstate, PyTypeObject *type, PyObject *target, PyObject *attr_name,
    ...
    PyObject *dict = NULL;
    PyObject **dict_pointer = NULL;

#if defined(__NUITKA_DMA_ACTIVE__) && defined(NUITKA_HAS_FIXED_DICT_OFFSET)
    // Compile-time offset: no tp_dictoffset load.
    dict_pointer = (PyObject **)((char *)target + (NITRO_FIXED_OFFSET));
#elif defined(__NUITKA_DMA_ACTIVE__) && defined(NUITKA_MANAGED_DICT)
    // Managed dict whose offset was not probed: _PyObject_GetDictPtr handles it.
    dict_pointer = _PyObject_GetDictPtr(target);
#else
    Py_ssize_t dict_offset = type->tp_dictoffset;
    ...
    if (dict_offset != 0) {
        ...
        dict_pointer = (PyObject **)((char *)target + dict_offset);
    }
#endif
